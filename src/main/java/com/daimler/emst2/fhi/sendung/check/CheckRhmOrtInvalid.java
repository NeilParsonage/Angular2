...
public class CheckRhmOrtInvalid extends AbstractCheckOrtError {
    ...
    protected boolean isOrtSendAllowed(SendContext pContext) {
        boolean isAllowed = isOrtAllowedForParameters(pContext, OrtCheckEnum.RHM_AUSFALL_ERLAUBT, OrtTypEnum.RHM);
        isAllowed &= isOrtAllowedForParameters(pContext, OrtCheckEnum.RHM_AUSFALL_ERLAUBT, OrtTypEnum.FHS);
        return isAllowed;
    }
}
